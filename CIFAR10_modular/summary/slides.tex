...
\begin{frame}{The task: \texttt{ConcatCIFAR10}}
  \centering
  \includegraphics[width=0.60\linewidth]{task_examples}
  \begin{itemize}\small
...
% --- 11. Final-epoch eigenspectrum -------------------------------------
\begin{frame}{Final-epoch eigenspectrum: $\lambda_1 \ldots \lambda_{10}$ for $N = 1 \ldots 8$}
  \centering
  \includegraphics[width=0.66\linewidth]{final_spectrum}
  \begin{itemize}\small
    \item Open circle marks position $k = N$ on each curve --- the last
      eigenvalue inside the ``module'' block.
    \item For $N \le 6$: $\lambda_1 \ldots \lambda_N$ all sit near zero, then
      the spectrum jumps at $k = N{+}1$ --- clear eigengap at the expected
      position.
    \item $N{=}7,8$: the in-block eigenvalues no longer hug zero and the
      jump at $k{=}N{+}1$ is shallower --- consistent with incomplete
      modularisation within the 100-epoch budget.
  \end{itemize}
\end{frame}
...
\begin{frame}{Visualising the learned modularity: $|W_{fc2}|$ heatmaps}
  \centering
  \includegraphics[width=0.6\linewidth]{fc2_heatmaps_grid}
  \begin{itemize}\small
