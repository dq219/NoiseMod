...
The [CIFAR10](./CIFAR10) folder extends the modularity investigation beyond synthetic tasks to a "film-roll" CIFAR-10 problem: each input is a horizontal concatenation of D independent CIFAR-10 frames and the target is the corresponding D-tuple of labels. [data.py](./CIFAR10/data.py) provides a `ConcatCIFAR10` dataset that wraps `torchvision.datasets.CIFAR10` and stitches D frames into a `(3, 32, 32*D)` tensor with a length-D label vector. [model.py](./CIFAR10/model.py) provides `ModularResNet18`, a CIFAR-style ResNet-18 whose final pooling preserves D positions along the width and whose fully connected layer is scaled to `(512*D) -> (10*D)` to emit one logit vector per frame.
